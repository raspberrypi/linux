	...
	struct snd_pcm_hw_params *params)
{
  struct snd_soc_pcm_runtime *rtd = substream->private_data;
	...
	int ret = snd_soc_dai_set_fmt(rtd->codec_dai, SND_SOC_DAIFMT_CBS_CFS|SND_SOC_DAIFMT_DSP_A|SND_SOC_DAIFMT_NB_NF);
	if (ret < 0)
		return ret;

	/* set cpu DAI configuration */
	ret = snd_soc_dai_set_fmt(rtd->cpu_dai, SND_SOC_DAIFMT_CBM_CFM|SND_SOC_DAIFMT_DSP_A|SND_SOC_DAIFMT_NB_NF);
	if (ret < 0)
		return ret;
...
static int audioinjector_octo_trigger(struct snd_pcm_substream *substream, int cmd){
  int mult[3];
  mult[0]=0;
  mult[1]=0;
  mult[2]=0;
  ...
  gpiod_set_array_value_cansleep(mult_gpios->ndescs, mult_gpios->desc, mult);

	return 0;
}

static struct snd_soc_ops audioinjector_octo_soundcard_ops = {
	.hw_params = audioinjector_octo_hw_params,
	.trigger = audioinjector_octo_trigger,
};

static struct snd_soc_dai_link audioinjector_octo_soundcard_dai[] = {
	{
		.name = "AudioInjector Octo",
		.stream_name = "AudioInject-HIFI",
		.codec_dai_name = "cs42448",
		.ops = &audioinjector_octo_soundcard_ops,
...
static struct snd_soc_card snd_soc_audioinjector_octo = {
	...
	.dai_link = audioinjector_octo_soundcard_dai,
	.num_links = ARRAY_SIZE(audioinjector_octo_soundcard_dai),
...
static int audioinjector_octo_soundcard_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		struct snd_soc_dai_link *dai = &audioinjector_octo_soundcard_dai[0];
		struct device_node *i2s_node = of_parse_phandle(pdev->dev.of_node,
								"i2s-controller", 0);
		struct device_node *codec_node = of_parse_phandle(pdev->dev.of_node,
								"codec", 0);
...
static int audioinjector_octo_soundcard_remove(struct platform_device *pdev)
{
...
static const struct of_device_id audioinjector_octo_soundcard_of_match[] = {
	{ .compatible = "ai,audioinjector-octo-soundcard", },
	{},
};
MODULE_DEVICE_TABLE(of, audioinjector_octo_soundcard_of_match);

static struct platform_driver audioinjector_octo_soundcard_driver = {
       .driver         = {
		.name   = "audioinjector-audio",
		.owner  = THIS_MODULE,
		.of_match_table = audioinjector_octo_soundcard_of_match,
       },
       .probe          = audioinjector_octo_soundcard_probe,
       .remove         = audioinjector_octo_soundcard_remove,
};

module_platform_driver(audioinjector_octo_soundcard_driver);
