				       ...
				       struct v4l2_format *f)
{
	...
	unicam_dbg(3, dev, "%s: fourcc: " v4l2_fourcc_conv " size: %dx%d bpl:%d img_size:%d\n",
		   __func__,
		   v4l2_fourcc_args(f->fmt.pix.pixelformat),
				...
				struct v4l2_format *f)
{
	...
	unicam_dbg(3, dev, "%s %dx%d, mbus_fmt %08X, V4L2 pix " v4l2_fourcc_conv ".\n",
		   ...
		   v4l2_fourcc_args(dev->v_fmt.fmt.pix.pixelformat));
...
static void unicam_async_unbind(struct v4l2_async_notifier *notifier,
				...
				struct v4l2_async_subdev *asd)
{
	...
	unicam_info(unicam, "Removing %s\n",
		    video_device_node_name(&unicam->video_dev));
...
static int of_unicam_connect_subdevs(struct unicam_device *dev)
{
	...
	case V4L2_MBUS_CSI2_DPHY:
		for (lane = 0;
		     ...
		     lane++) {
			if (ep->bus.mipi_csi2.data_lanes[lane] != lane + 1) {
				unicam_err(dev, "Incompatible data lane config\n");
				goto cleanup_exit;
			}
		}
		dev->max_data_lanes = ep->bus.mipi_csi2.num_data_lanes;
	...
	case V4L2_MBUS_CCP2:
		if (ep->bus.mipi_csi1.clock_lane != 0 ||
			...
			unicam_err(dev, "Incompatible lane config\n");
			goto cleanup_exit;
		}
	...
	default:
		...
		unicam_err(dev, "Bus type %d is not a CSI2 or CCP2 device\n",
			   ep->bus_type);
		goto cleanup_exit;
	}
	...
	unicam_dbg(1, dev, "sensor_node is %s\n", sensor_node->name);
	...
	if (ret) {
		unicam_err(dev, "failed adding subdev to notifier - ret %d\n",
			   ret);
		goto cleanup_exit;
	}
