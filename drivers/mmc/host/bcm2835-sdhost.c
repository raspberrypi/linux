...
#define CMD_GO_IDLE_STATE               (0|SDCMD_NO_RESPONSE)
#define CMD_SEND_OP_COND                (1)
#define CMD_ALL_SEND_CID                2
#define CMD_SEND_RELATIVE_ADDR          3
#define CMD_SWITCH                      (6|SDCMD_BUSYWAIT)
#define CMD_SELECT_CARD                 (7|SDCMD_BUSYWAIT)
#define CMD_SEND_IF_COND                8  // High Capacity SD Card specific
#define CMD_SEND_EXT_CSD                (8|SDCMD_READ_CMD)  // MMC micro / mobile / plus specific
#define CMD_SEND_CSD                    9
#define CMD_SEND_CID                    10
#define CMD_STOP_TRANSMISSION           (12|SDCMD_BUSYWAIT)
#define CMD_SEND_STATUS                 13
#define CMD_GO_INACTIVE_STATE           (15|SDCMD_NO_RESPONSE)
#define CMD_SET_BLOCKLEN                16
#define CMD_READ_SINGLE_BLOCK           (17|SDCMD_READ_CMD)
#define CMD_READ_MULTIPLE_BLOCK         (18|SDCMD_READ_CMD)
#define CMD_WRITE_BLOCK                 (24|SDCMD_WRITE_CMD)
#define CMD_WRITE_MULTIPLE_BLOCK        (25|SDCMD_WRITE_CMD)
#define CMD_ERASE_WR_BLOCK_START        32
#define CMD_ERASE_WR_BLOCK_END          33

#define CMD_ERASE_WR_BLOCK_START_MMC    35
#define CMD_ERASE_WR_BLOCK_END_MMC      36

#define CMD_ERASE                       (38|SDCMD_BUSYWAIT)
...
static void bcm2835_sdhost_wait_write_complete(struct bcm2835_host *host)
{
	...
	while (1) {
		...
		if (timediff > 5000000) {
			...
			pr_err(" dma_complete - still waiting after %dus\n",
			       timediff);
	...
	if ((after.tv_sec - start_time.tv_sec) > 10) {
		pr_debug(" dma_complete - max wait %dus, total %dus\n",
...
static void bcm2835_sdhost_transfer_complete(struct bcm2835_host *host)
{
	...
	if (data->stop &&
		...
		if (!(data->stop->flags & MMC_RSP_BUSY))
...
static void bcm2835_sdhost_finish_command(struct bcm2835_host *host)
{
	...
	if (host->cmd == host->mrq->sbc) {
		...
		/* Finished CMD12 */
		pr_debug("SDDEBUG: finish_command stop tasklet_schedule\n");
		tasklet_schedule(&host->finish_tasklet);
	} else {
		/* Processed actual command. */
		host->cmd = NULL;
		if (!host->data) {
			pr_debug("SDDEBUG: finish_command tasklet_schedule\n");
			tasklet_schedule(&host->finish_tasklet);
					 ...
					 u32 blk_pos, int blk_size)
{
	...
	*/
	if ((direction == MMC_DATA_READ) &&
	    ((blk_pos + blk_size) == card->csd.capacity))
		blk_size--;
