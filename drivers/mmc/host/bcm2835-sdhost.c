...
static void log_init(u32 bus_to_phys)
{
  ...
  sdhost_log_buf = dma_zalloc_coherent(NULL, LOG_SIZE, &sdhost_log_addr,
				       GFP_KERNEL);
  if (sdhost_log_buf) {
    pr_info("sdhost: log_buf @ %p (%ld)\n",
	    sdhost_log_buf, (long int) sdhost_log_addr);
...
>>>>>>> 6025d8e1... Added spi-bcm2708 kernel driver -- compilation succeeds and driver now works as
