...
static int gpio_fsm_set_soft(struct gpio_fsm *gf,
...
static int gpio_fsm_set(struct gpio_chip *gc, unsigned int off, int val)
{
	struct gpio_fsm *gf;

	gf = gpiochip_get_data(gc);
	if (off < gf->num_soft_gpios)
		return gpio_fsm_set_soft(gf, off, val);
	return -EINVAL;
}
