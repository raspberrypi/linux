...
static inline u32 bcm2708_rd(struct bcm2708_spi *bs, unsigned reg) { return readl(bs->base + reg); }

static inline void bcm2708_wr(struct bcm2708_spi *bs,
			      unsigned reg, u32 val)
{ writel(val, bs->base + reg); }

static inline void bcm2708_rd_fifo(struct bcm2708_spi *bs, int len)
{
  u8 byte = bcm2708_rd(bs, SPI_FIFO);
  for (; len--; byte = bcm2708_rd(bs, SPI_FIFO)) if (bs->rx_buf) *bs->rx_buf++ = byte;;
}
...
static inline void bcm2708_wr_fifo(struct bcm2708_spi *bs, int len)
{
  ...
  if (unlikely(bcm2708_rd(bs, SPI_CS) & SPI_CS_LEN)) {
    ...
    for (;len; bcm2708_wr(bs, SPI_FIFO, val), bs->len -= 2, len -= 2) {
      if (bs->tx_buf) {
...
{
  struct bcm2708_spi *bs = spi_master_get_devdata(master);
  int cdiv; u32 bus_hz, cs = 0; // unsigned long == unsinged in on ARM
  ...
  if (hz >= bus_hz) {
    ...
    if (cdiv > 65536) {
      dev_dbg(dev,
	      "setup: %d Hz too slow, cdiv %u; min %d Hz\n",
...
MODULE_AUTHOR("Chris Boot <bootc@bootc.net>, Hunter Allen <allen286@purdue.edu>");
