 ...
 * The first occurs 1 cycle before the middle of the line. The second
 ...
 * round-trip delay is about 7 cycles when pins are not heavily loaded.
 *
 * PIO code           ; Notional time % 1000-cycle period
 * --------           ; ---------------------------------
 * 0: wait 1 gpio 3   ; 999..  7
 * 1: mov x, y        ;   7..  8
 * 2: jmp x--, 2      ;   8..498    (Y should be T/2 - 11)
 * 3: irq set 1       ; 498..499
 * 4: mov x, y    [8] ; 499..508
 * 5: jmp x--, 5      ; 508..998
 * 6: irq set 1       ; 998..999
 ...
 * half-line boundary to improve VSync start accuracy) and VBP in 0..254.
 */
				  ...
				  struct drm_display_mode const *mode)
{
	u16 instructions[] = {  /* This is mutable */
		...
		0x6028, // 13: out    x, 8        side 0     ;  VSYNC=0; x = VBPLen
		0x0010, // 14: jmp    16          side 0     ;  while (x--) {
		0x20c1, // 15: wait   1 irq, 1    side 0     ;    @halfline
		0x004f, // 16: jmp    x--, 15     side 0     ;  }
		0x00c0, // 17: jmp    pin, 0      side 0     ;  if (DE) reset phase
		0x0013, // 18: jmp    19          side 0     ;  <modify for -DE fixup>
		0x00e1, // 19: jmp    !osre, 1    side 0     ;  if (!phase) goto main
		//     .wrap                                 ;  goto top
	};
	...
	/* Compute half-line time constant and start the timer SM */
	tc = (u32)clk_get_rate(dpi->clocks[RP1DPI_CLK_DPI]);
	if (!tc)
		tc = 1000u * mode->clock;
	tc = ((u64)mode->htotal * (u64)sysclk + tc) / (u64)(2ul * tc);
	...
	if (dpi->de_inv) {
		...
		instructions[17] = 0x00d3;
		instructions[18] = 0x0000;
	}
