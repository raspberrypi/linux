...
struct ed_panel {
	...
	const struct drm_display_mode *mode;
	const struct edatec_panel *panel;
...
static const struct edatec_panel cm0_panel = {
		.cpu_type = CPU_CM0,
		.mode = &ed_panel_7_0_cm0_mode,
};

static const struct edatec_panel cm4_panel = {
		.cpu_type = CPU_CM4,
		.mode = &ed_panel_7_0_mode,
};
...
static int ed_panel_probe(struct i2c_client *i2c)
{
	...
	ts->panel = of_device_get_match_data(dev);
	ts->mode = ts->panel->mode;
	if (!ts->mode)
		return -EINVAL;
	...
	ts->dsi->mode_flags = (MIPI_DSI_MODE_VIDEO | MIPI_DSI_MODE_VIDEO_SYNC_PULSE |
		MIPI_DSI_MODE_LPM);
