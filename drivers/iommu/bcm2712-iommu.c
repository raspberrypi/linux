...
#define TABLES_LXPAGE_SHIFT   (IOMMU_PAGE_SHIFT + LX_PAGEWORDS_SHIFT)
 ...
 */
static u32 bcm2712_iommu_getpage(struct bcm2712_iommu *mmu, u32 **ptr)
{
...
static int bcm2712_iommu_init(struct bcm2712_iommu *mmu)
{
	 ...
	 */
	u = bcm2712_iommu_getpage(mmu, &mmu->top_table);
	if (!u)
		return -ENOMEM;
	MMU_WR(MMMU_PT_PA_BASE_OFFSET,
	       u - ((mmu->aperture_base - mmu->dma_iova_offset) >> L1_AP_BASE_SHIFT));
	u = bcm2712_iommu_getpage(mmu, &mmu->default_page);
	if (!u) {
			     ...
			     int prot, gfp_t gfp, size_t *mapped)
{
	 ...
	 * A failure here will cause the entire map() call to fail.
	 */
	spin_lock_irqsave(&mmu->hw_lock, flags);
	for (p = iova >> TABLES_LXPAGE_SHIFT;
	     p <= (iova + bytes - 1) >> TABLES_LXPAGE_SHIFT; p++) {
		if (!mmu->tables[p]) {
			unsigned int p1, u;

			u = bcm2712_iommu_getpage(mmu, &mmu->tables[p]);
			if (!u) {
			...
			u |= MMMU_PTE_VALID;
			for (p1 = p << (PAGE_SHIFT - IOMMU_PAGE_SHIFT);
			     p1 < (p + 1) << (PAGE_SHIFT - IOMMU_PAGE_SHIFT); p1++) {
				mmu->top_table[p1] = u++;
			}
			mmu->dirty_top = true;
		}
	...
	/* Now fill in the level-2 tables */
	for (p = iova >> IOMMU_PAGE_SHIFT;
	     p < (iova + bytes) >> IOMMU_PAGE_SHIFT; p++) {
				  ...
				  struct iommu_iotlb_gather *gather)
{
	...
	/* DMA address -> address relative to region mapped by tables */
	iova -= mmu->aperture_base;

	/* Clear table entries, this marks the addresses as illegal */
	for (p = iova >> IOMMU_PAGE_SHIFT;
	     p < (iova + bytes) >> IOMMU_PAGE_SHIFT;
	     p++) {
		u32 *tbl = mmu->tables[p >> LX_PAGEWORDS_SHIFT];

		if (tbl && tbl[p & LX_PAGEWORDS_MASK]) {
				    ...
				    unsigned long iova, size_t size)
{
	...
	spin_lock_irqsave(&mmu->hw_lock, flags);
	for (i = (iova - mmu->aperture_base) >> TABLES_LXPAGE_SHIFT;
	     i <= (iova_end - mmu->aperture_base - 1) >> TABLES_LXPAGE_SHIFT; i++) {
		if (mmu->tables[i]) {
			dma_sync_single_for_device(mmu->dev,
						   virt_to_phys(mmu->tables[i]),
						   PAGE_SIZE, DMA_TO_DEVICE);
		}
	}
	if (mmu->dirty_top) {
		dma_sync_single_for_device(mmu->dev,
					   virt_to_phys(mmu->top_table),
					   PAGE_SIZE, DMA_TO_DEVICE);
		mmu->dirty_top = false;
	}
	 ...
	 */
	if (mmu->nmapped_pages == 0 || iova_end >= iova + (1ul << 26)) {
		MMU_WR(MMMU_CTRL_OFFSET,
		       MMMU_CTRL_CAP_EXCEEDED_ABORT_EN    |
		       MMMU_CTRL_PT_INVALID_ABORT_EN      |
		       MMMU_CTRL_PT_INVALID_EN            |
		...
		iova_end -= mmu->dma_iova_offset;
		for (p4 = iova >> (IOMMU_PAGE_SHIFT + 2);
		     p4 < (iova_end + 3 * IOMMU_PAGE_SIZE) >> (IOMMU_PAGE_SHIFT + 2);
		     p4++) {
...
{
	phys_addr_t addr = (phys_addr_t)(-EINVAL);
	struct bcm2712_iommu *mmu = domain_to_mmu(domain);
...
static int bcm2712_iommu_probe(struct platform_device *pdev)
{
	...
	num_pages = mmu->aperture_end >> TABLES_LXPAGE_SHIFT;
	mmu->aperture_end += mmu->aperture_base;
...
static void bcm2712_iommu_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < (mmu->aperture_end - mmu->aperture_base) >> TABLES_LXPAGE_SHIFT; i++)
